...
### http://www.acajoom.com/license.php
 ...
 $_MAMBOTS->registerFunction('acajoombot_editabs', 'acajoombot_content_editab');
 $_MAMBOTS->registerFunction('acajoombot_transformall', 'acajoombot_content_transformall');
 $_MAMBOTS->registerFunction('acajoombot_transformall', 'acajoombot_jcalpro_transformall');
 $_MAMBOTS->registerFunction('acajoombot_transformfinal', 'acajoombot_class_transformfinal');
	...
	if($content_item->created_by_alias == ''){$content_item->created_by_alias = $content_item->name;}
	 ...
	 if (!$erro->E(__LINE__ ,  '8011')	) {
		...
		if(get_magic_quotes_runtime()) {
            ...
            $content_item->sectionid = stripslashes($content_item->sectionid);
            $content_item->catid = stripslashes($content_item->catid);
		}
		return $content_item;
	}
 }
